...
class SendReq extends Component {
    ...
    render() {
        let upitnik = null;
        if (this.state.prikaziUpitnik) upitnik = (
            <div>
                <div>Korisnik postoji u bazi, da li zelite da prethodno arhivirani tvitovi budu ukljuceni u ovu analizu?</div>
                <button onClick={this.bezArhiviranihPodatakaHandler} >NE</button>
                <button onClick={this.saArhiviranimPodatacimaHandler} >DA</button>
            </div>
        );
        ...
        return (
            <div className="NewPost" style={{backgroundColor: '#404040'}}>
                ...
                <button onClick={() => this.postDataHandler(this.state.username)} >Retrieve tweets</button>
                {upitnik}
